...
class B1_CNN(nn.Module):
    def __init__(self):
        super(B1_CNN, self).__init__()
        self.dense = nn.Sequential(nn.Conv2d(4, 4, 3),
                                   nn.PReLU(),
                                   nn.BatchNorm2d(4),
                                   nn.Conv2d(4, 8, 3),
...
class B1(object):
        ...
        if self.use_CNN:
            ...
            for epoch in range(10):
                ...
                if np.mean(running_acc) > best_acc:
                    ...
                    pt.save(self.model.state_dict(), 'B1_CNN.pkl')
            return best_acc
        else:
    ...
    def test(self, test):
        if self.use_CNN:
            ...
            # load the model 
            self.model.load_state_dict(pt.load('B1_CNN.pkl'))
            test_acc = []
