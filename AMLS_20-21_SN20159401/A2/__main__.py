...
class A2_CNN(nn.Module):
    def __init__(self):
        super(A2_CNN, self).__init__()
...
class A2(object):
        ...
        if self.use_CNN:
            ...
            for epoch in range(20):
                ...
                if np.mean(running_acc) > best_acc:
                    ...
                    pt.save(self.model.state_dict(), 'A2_CNN.pkl')
            return best_acc
        else:
    ...
    def test(self, test):
        if self.use_CNN:
            self.model.load_state_dict(pt.load('A2_CNN.pkl'))
            test_acc = []
